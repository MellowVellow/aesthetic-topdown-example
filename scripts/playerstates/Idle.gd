...
func enter(_msg := {}) -> void:
	animation_tree.get("parameters/playeranims/playback").travel("idle")
	
	
func physics_update(_delta: float) -> void:
	animation_tree.set("parameters/playeranims/idle/blend_position", player.last_input_vector)
	
	if player.input_vector != Vector2.ZERO:
		state_machine.transition_to("Walk")
	if Input.is_action_pressed("sprint") and player.input_vector != Vector2.ZERO:
